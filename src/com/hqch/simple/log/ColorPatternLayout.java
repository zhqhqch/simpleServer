...
public class ColorPatternLayout extends PatternLayout {
	...
	private static final String BG_NULL = "NULL";

	private String fatalErrorColor = PREFIX + ATTR_DIM + SEPARATOR + FG_RED
			+ SUFFIX;
	private String errorColor = PREFIX + ATTR_REVERSE + SEPARATOR + FG_RED + SUFFIX;
	private String warnColor = PREFIX + ATTR_DIM + SEPARATOR + FG_MAGENTA
			+ SUFFIX;
	private String infoColor = PREFIX + ATTR_DIM + SEPARATOR + FG_GREEN
			+ SUFFIX;
	private String debugColor = PREFIX + ATTR_DIM + SEPARATOR + FG_WHITE
			+ SUFFIX;
	...
	private String makeFgString(String fgColorName) {
		...
		System.out.println("log4j: ColoredPatternLayout Unsupported FgColor "
				+ fgColorName);
		return "-1";
	}

	private String makeBgString(String bgColorName) {
		if (bgColorName.toLowerCase().equals("null")) {
			return BG_NULL;
		} else if (bgColorName.toLowerCase().equals("black")) {
			return BG_BLACK;
		...
		System.out.println("log4j: ColoredPatternLayout Unsupported BgColor "
				+ bgColorName);
		return "-1";
	}
	...
	private String makeAttributeString(String attributeName) {
		...
		System.out.println("log4j: ColoredPatternLayout Unsupported Attribute "
				+ attributeName);
	...
	private String makeColorString(String colorName) {
		...
		result.append(PREFIX + attr + SEPARATOR + fgColor);
		if (!bgColor.toLowerCase().equals("null")) {
			result.append(SEPARATOR + bgColor + SUFFIX);
		} else {
